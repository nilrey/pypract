...
class FileData():
    def __init__(self, project_id, dataset_id, fname) -> None:
        self.project_id = project_id
        self.dataset_id = dataset_id
        self.file_name = fname 
        self.file_id = self.get_file_id(fname)
    ...
    def get_file_id(self, fname):
        # stmt = text("SELECT f.id FROM files f \
        # WHERE f.dataset_id = :dataset_id, f.name = :fname")
        # resp = dbq.select_wrapper(stmt, {"dataset_id" : self.dataset_id, "fname" : fname} )
        # # resp = self.execute_query( self.create_connection() , query )
        # return resp
...
class JsonSaveDB():
    ...
    def set_file_id(self, fname = "1ca4627c-7b58-11ef-b565-0242ac140002"):
        self.qparams['file_id'] = fname #self.get_file_id(fname)
    ...
    def select_all_chain_markups(chain_id):
        stmt = text("SELECT * FROM chains c WHERE c.origin_id = :chain_id")
        resp = dbq.select_wrapper(stmt, {"chain_id" : chain_id } )
        return resp


    def select_single_chain_markups(chain_id, markup_id):
        stmt = text("SELECT * FROM markups_chains mc \
        WHERE mc.chain_id = :chain_id and mc.markup_id = :markup_id")
        resp = dbq.select_wrapper(stmt, {"chain_id" : chain_id, "markup_id" : markup_id} )
        # resp = self.execute_query( self.create_connection() , query )
        return resp
    ...
    def parse_content(self, content):
        ...
        for f in content['files']:
            for chain in f['file_chains'] :
                chain_query_values = self.select_all_chain_markups()
    ...
    def proceed_file(self, filename, fd):
        time.sleep(1)
        self.set_file_id( fd.get_file_name() )
        print(self.qparams['file_id'])
    ...
    def loop_files(self, files):
        for filename in files:
            fd = FileData(self.project_id, self.dataset_id, filename)
            threading.Thread(target=self.proceed_file, args=(filename, fd)).start()
